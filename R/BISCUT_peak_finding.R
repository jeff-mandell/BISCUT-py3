...
#' 
#' @param breakpoints Either the output list from calculate_breakpoints(), or a path to a 
#'   breakpoint file directory generated by the same function.
#' @param results_dir The path for a results directory that will be created on running. When NULL
...
#' \item overlapping_peaks: Information about pairs of peaks that cover the same genomic territory.
...
#' \item peak_plot_data: Data that can be used to make peak plots (plot_peaks()).
#' \item wide_format: An alternative wide results format.
#' \item info: Additional BISCUT run information, such as parameter choices.
#' }
#' @export
do_biscut = function(breakpoints, results_dir = NULL, use_precalculated_background = FALSE,
  ...
  using_breakpoint_dir = FALSE
  if(rlang::is_scalar_character(breakpoints)) {
    using_breakpoint_dir = TRUE
    if(! dir.exists(breakpoints)) {
      stop('Input breakpoints was type character (indicating a breakpoint file directory), but no such directory exists.')
    }
    breakpoints = normalizePath(breakpoints)
    breakpoints = load_breakpoint_files(breakpoints, telcent_thres = telcent_thres)
  } else if(! is.list(breakpoints) || is.null(breakpoints$tel) || is.null(breakpoints$cent)) {
    stop("Input breakpoints not in expected format. They should be a list from calculate_breakpoints(), ",
  ...
  writing_results = ! is.null(results_dir)
  if(is.null(results_dir)) {
    results_dir = file.path(tempdir(), paste0(as.numeric(Sys.time()), '_biscut'))
  }
  ...
  on.exit({
    if(! finished_processing || ! writing_results) {
      unlink(results_dir, recursive = T)
      # No need to tell the user when deleting a temp directory.
      if(writing_results) {
  ...
  results = pbapply::pblapply(1:nrow(run_args),
                                 function(i) {
                                   ...
                                   do_arm_gistic(arm = run_args$a[i],
                                                 ...
                                                 probes = probes[amp_del == direc & telcent == run_args$tc[i] & arm == run_args$a[i],
                                                                 .(Sample, percent, start, end)],
                                                 n = n_bootstrap,
